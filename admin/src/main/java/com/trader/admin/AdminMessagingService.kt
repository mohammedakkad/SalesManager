...
class AdminMessagingService : FirebaseMessagingService() {
    ...
    fun checkExpiringMerchants() {
        ...
        // نحفظ الـ context قبل الدخول في الـ callback
        // لأن الـ Service قد يكون destroyed عند تنفيذ الـ callback
        val appContext = applicationContext ?: return

        db.collection("merchants").get().addOnSuccessListener {
            snapshot ->
    ...
    private fun showNotification(context: android.content.Context, title: String, body: String) {
