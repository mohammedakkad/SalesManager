    ...
    NavHost(navController = navController, startDestination = start) {
        composable(AdminScreen.Login.route) {
            LoginScreen(onAuthenticated = {
                navController.navigate(AdminScreen.Dashboard.route) { popUpTo(AdminScreen.Login.route) { inclusive = true } }
            })
        }
        ...
        composable(AdminScreen.MerchantDetail.route, listOf(navArgument("merchantId") { type = NavType.StringType })) {
            MerchantDetailScreen(merchantId = it.arguments!!.getString("merchantId")!!, onNavigateUp = { navController.navigateUp() })
        }
