...
data class InventoryListUiState(
    ...
    val filtered: List<ProductWithUnits> get() = products.filter { p ->
    ...
    val lowStockCount: Int get() = products.count { it.isLowStock }
    val outOfStockCount: Int get() = products.count { it.isOutOfStock }
}
...
class InventoryListViewModel(
    ...
    }.stateIn(viewModelScope, SharingStarted.WhileSubscribed(5_000), InventoryListUiState())

    fun setQuery(q: String)          { _query.value = q }
    fun setFilter(f: StockFilter)    { _filter.value = f }
    fun showScanner(show: Boolean)   { /* handled by UI state in screen */ }
